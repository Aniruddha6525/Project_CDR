    ...
    return (
        <div className="space-y-6">
            ...
            <div className="space-y-2">
                <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2">
                        <span className={clsx("text-xs transition-colors", !useManualTranscript ? "text-primary font-bold" : "text-muted-foreground")}>Auto-Generate</span>
                        <button
                        ...
                        >
                            <span
                                className={clsx(
                                    "inline-block h-3 w-3 transform rounded-full bg-primary-foreground transition-transform",
                                    useManualTranscript ? 'translate-x-5' : 'translate-x-1'
                                )}
                            />
                        </button>
                        <span className={clsx("text-xs transition-colors", useManualTranscript ? "text-primary font-bold" : "text-muted-foreground")}>Manual Input</span>
                    </div>
                </div>
                    ...
                    {useManualTranscript ? (
                        ...
                        >
                            <textarea
                                ...
                                className="w-full h-48 bg-secondary/20 backdrop-blur-xl border border-border/50 rounded-2xl p-5 text-sm font-medium text-foreground focus:border-primary/50 focus:ring-2 focus:ring-primary/20 transition-all resize-none shadow-inner"
                            />
                            <div className="absolute bottom-5 right-5 text-[10px] font-bold tracking-widest uppercase text-muted-foreground/60 bg-background/80 px-2 py-1 rounded-md border border-border/50">
            ...
            <button
                ...
                disabled={!file}
                className={clsx(
                    "w-full py-4.5 rounded-2xl font-bold text-lg tracking-wide transition-all duration-300 flex items-center justify-center gap-2 shadow-sm border border-transparent",
                    file
            ...
            >
                {file ? (
                    ...
                    <>Upload Audio to Begin <AlertCircle className="w-5 h-5" /></>
                )}
            </button>
        </div>
    );
